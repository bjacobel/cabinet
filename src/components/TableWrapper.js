...
const headerize = (text) => (
  <span className={classnames({ [smol]: text.length > 20 })}>
    {text.split(' ').map((textchunk, ind) => (
      <span className={antiskew} key={ind}>{`${textchunk} `}</span> // eslint-disable-line react/no-array-index-key, max-len
    ))}
  </span>
);
      ...
      <div className={table}>
        ...
        >
          ...
          <Column // States of senators
            width={50}
            header={() => (
              <Cell className={cell} style={{ paddingTop: '6px' }}>
                {headerize('State')}
              </Cell>
            )}
            cell={(props) => {
              if (senators) {
                const senator = senators[props.rowIndex];
                return <Cell className={classnames(cell, partyClass(senator.party))}>{senator.state}</Cell>;
              } else {
                return <Cell />;
              }
            }}
          ...
          {Object.entries(voteRecords).map(([voteId, votersForVote]) => (
            <Column // The vote on each nominee
              ...
              width={42}
              header={() => (
                <Cell className={cell} data-rh={votes[voteId].description} data-rh-at="bottom">
